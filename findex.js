...
function appendData(data){
    ...
    data.map((el,i)=>{     
        ...
        p.textContent=`Name:- ${el.title}`;
        p2.textContent=`Price:${el.price}`;
    
        card.append(img,ratt,p,p2);
        document.querySelector("#container").append(card);
    })
}


let fil= document.getElementById('filterr');
  fil.addEventListener('change', ()=> {
      let selectedValue = fil.value;

 let filteredData = products.filter((el,index)=>{
    return selectedValue == el.category
 })
